...
def save(self, *args, **kwargs):
    """Incrémente version à chaque modification."""
    if self.pk:
        self.version += 1
...
**Rôle:** Envoie code 6 chiffres par email ou SMS

**Logique:**
1. Génère code 6 chiffres (`secrets.choice`)
2. Détermine canal (email/phone selon `preferred_channel`)
3. Stocke code dans cache Redis : `double_opt_{client_token}` → TTL 120s
4. Envoie via email ou SMS backend
...
### Fichier: `core_data/utils/sms_backend.py`
