...
def send_verification_code(user):
    ...
    # Stocker dans le cache (Redis) avec TTL de 3 minutes
    cache_key = f'email_verification_{user.id}'
    cache.set(cache_key, code, timeout=180)  # 180 secondes = 3 minutes
...
def check_profile_completion(user):
    ...
    return {
        'pass_onboading':profile.pass_onboading,
...
def send_password_reset_code(email):
    ...
    cache.set(cache_key, code, timeout=180)  # 3 minutes
