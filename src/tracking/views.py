...
class TrackingViewSet(viewsets.ViewSet):
    ...
    def heartbeat(self, request):
        ...
        Appelé au premier chargement de status.html.
        Ensuite c'est Celery qui prend le relais via l'API MikroTik.
        """
    ...
    def end(self, request):
        """Ferme une session depuis logout.html via sendBeacon."""
...
class MikroTikRouterViewSet(viewsets.ModelViewSet):
    """
    CRUD sur les routeurs MikroTik de l'owner connecté.
    Action supplémentaire : test_connection pour valider la config.
    """
    serializer_class   = MikroTikRouterSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        return MikroTikRouter.objects.filter(owner=self.request.user)
    ...
    def test_connection(self, request, pk=None):
        """
        Teste la connexion à un routeur et retourne le résultat immédiatement.
        Appelé depuis le dashboard quand l'owner veut vérifier sa config.
        ...
        """
        from .mikrotik_api import test_router_connection
    ...
    def sync_now(self, request, pk=None):
        """
        Déclenche une synchronisation immédiate pour ce routeur.
        Utile pour tester ou forcer une mise à jour depuis le dashboard.
        """
...
class ConnectionSessionViewSet(mixins.ListModelMixin,
    ...
    def get_queryset(self):
        qs = ConnectionSession.objects.filter(owner=self.request.user) \
            .select_related('client', 'ticket_plan', 'router')
