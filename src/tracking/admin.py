...
class TicketPlanAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Limites de données (optionnel)', {
            'fields': ('download_limit_mb', 'upload_limit_mb'),
            'description': "Vide = illimité. Ces valeurs servent au matching automatique avec les sessions MikroTik.",
        }),
        ('État', {
    ...
    @admin.display(description='Limites D/U')
    def limits_display(self, obj):
        d = f"{obj.download_limit_mb} MB" if obj.download_limit_mb else "∞"
        u = f"{obj.upload_limit_mb} MB" if obj.upload_limit_mb else "∞"
        return f"{d} / {u}"
