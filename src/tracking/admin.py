...
Gestion des plans tarifaires (TicketPlan) et des sessions WiFi
(ConnectionSession), avec actions en bulk, filtres, recherche et export CSV.
"""
import csv
...
class ConnectionSessionAdmin(admin.ModelAdmin):
    """Administration des sessions WiFi capturées via les scripts MikroTik."""
    ...
    fieldsets = (
        ('Identification', {
            'fields': ('session_key', 'owner', 'client', 'ticket_plan', 'router'),
        }),
        ('Réseau MikroTik', {
            'fields': (
        ...
        ('État', {
            'fields': ('is_active', 'disconnect_cause', 'started_at', 'ended_at', 'last_heartbeat'),
        }),
    )
    ...
    def action_force_close(self, request, queryset):
        n = queryset.filter(is_active=True).update(
            ...
            disconnect_cause='forced-by-admin',
        )
    ...
    def action_export_csv(self, request, queryset):
        ...
        writer.writerow([
            ...
            'mac_address', 'ip_address', 'ticket_id', 'mikrotik_session_id',
            'plan', 'session_timeout_seconds',
            'started_at', 'ended_at', 'duration_seconds',
            'download_mb', 'upload_mb', 'total_mb',
            'is_active', 'disconnect_cause',
        ])
