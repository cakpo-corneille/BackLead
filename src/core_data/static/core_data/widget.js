                ...
                try {
                                ...
                                try {
                                    if (isConflict) {
                                        const resubmitResult = await fetchAPI(CONFIG.API_BASE + 'submit/', {
                                            method: 'POST',
                                            headers: { 'Content-Type': 'application/json' },
                                            body: JSON.stringify(Object.assign({}, submitBody, { verification_code: code }))
                                        });
                                        if (resubmitResult.client_token) {
                                            Storage.set('token_' + publicKey, resubmitResult.client_token);
                                        }
                                        showToast('success', 'Compte associe avec succes !');
                                        closeModal(overlay, maybeLoadScrapper);
                                    } else {
                                        await fetchAPI(CONFIG.API_BASE + 'confirm/', {
                                            method: 'POST',
                    ...
                    if (error.status === 400 && payloadErrors && typeof payloadErrors === 'object' && !Array.isArray(payloadErrors)) {
                        ...
                        if (error.status === 404) {
                            ...
                            errorMessage = 'Erreur serveur. Veuillez reessayer dans quelques instants.';
                        }
