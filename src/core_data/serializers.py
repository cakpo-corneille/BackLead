...
from .models import FormSchema, OwnerClient, ConflictAlert, ClientDevice
from tracking.serializers import get_tracking_snippet as get_full_tracking_snippet
...
class FormSchemaSerializer(serializers.ModelSerializer):
    ...
    tracking_snippet = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        read_only_fields = (
            'id', 'public_key',
            'created_at', 'updated_at',
            'integration_snippet', 'tracking_snippet', 'version',
            'logo_url',
            'media_host',
        )
    ...
    def get_tracking_snippet(self, obj):
        """
        Délègue la génération des 2 snippets RouterOS à l'app tracking.
        L'URL du backend est dérivée dynamiquement depuis request (même logique
        que get_integration_snippet pour le widget JS).
        Retourne un dict avec les clés : on_login, on_logout.
        """
        request = self.context.get('request')
        return get_full_tracking_snippet(str(obj.public_key), request)


# ============================================================================
# CLIENT DEVICE SERIALIZER
# ============================================================================

class ClientDeviceSerializer(serializers.ModelSerializer):
    class Meta:
        model = ClientDevice
        fields = ('id', 'mac_address', 'user_agent', 'first_seen', 'last_seen')
        read_only_fields = ('id', 'mac_address', 'user_agent', 'first_seen', 'last_seen')
...
class OwnerClientSerializer(serializers.ModelSerializer):
    ...
    """
    devices = ClientDeviceSerializer(many=True, read_only=True)
...
class ConflictAlertSerializer(serializers.ModelSerializer):
    ...
    existing_client_first_name = serializers.CharField(source='existing_client.first_name', read_only=True)
    existing_client_last_name  = serializers.CharField(source='existing_client.last_name', read_only=True)

    class Meta:
        model = ConflictAlert
        fields = (
            'id', 'existing_client',
            'existing_client_email', 'existing_client_phone',
            'existing_client_first_name', 'existing_client_last_name',
...
class SubmissionSerializer(serializers.Serializer):
    ...
    identity_confirmed = serializers.BooleanField(
        required=False,
        default=False,
        help_text="True si le client a confirmé 'Oui, c'est moi' lors d'un conflit d'identité."
    )
