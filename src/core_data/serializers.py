...
class FormSchemaSerializer(serializers.ModelSerializer):
    ...
    def get_tracking_snippet(self, obj):
        """
        Délègue la génération des snippets à l'app tracking.
        """
        request = self.context.get('request')
        return {
            'full': get_full_tracking_snippet(obj.public_key, request, mode='full'),
            'logout': get_full_tracking_snippet(obj.public_key, request, mode='logout')
        }
