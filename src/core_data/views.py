...
from core_data.services.dashboard.analytics import  analytics_summary, analytics_history # type: ignore
...
class LeadViewSet(mixins.ListModelMixin,
    ...
    def export(self, request):
        """
        Export CSV ou PDF de tous les clients (respecte les mêmes filtres que la liste).
        Query params:
            - format: 'csv' (défaut) ou 'pdf'
            - tous les filtres LeadFilter (is_verified, tags, search…)
        """
        from core_data.services.dashboard.export_service import export_csv, export_pdf
    ...
    def resend_verification(self, request, pk=None):
        ...
        if not lead.phone:
            return Response(
                {"detail": "Aucun numéro de téléphone enregistré pour ce client."},
                status=400,
            )

        success, message = resend_verification_code(lead)
...
class AnalyticsViewSet(viewsets.ViewSet):
    ...
    def summary(self, request):
        ...
        try:
            ...
            days = 7
        data = analytics_summary(request.user.id, days=days, date_from=date_from)
        return Response(data)

    @action(detail=False, methods=['get'], url_path='history')
    def history(self, request):
        """Récupère l'historique des 12 derniers mois."""
        data = analytics_history(request.user.id)
        return Response(data)
...
class PortalViewSet(viewsets.ViewSet):
    ...
    def submit(self, request):
        ...
        try:
            ...
            result = ingest(
                form_schema=data['_form_schema'],
                mac_address=data['mac_address'],
                payload=data['payload'],
                client_token=data.get('client_token'),
                verification_code=data.get('verification_code'),
                user_agent=user_agent,
                identity_confirmed=data.get('identity_confirmed', False)
            )
