...
class DoubleOptInTest(TestCase):
    ...
    def test_portal_submit_duplicate_email_success(self):
        """ALLOW strategy: même email, nouvel appareil → rattachement silencieux (pas de blocage)."""
        self.schema.opt = True
        ...
        # Deuxième appareil avec MÊME email — stratégie ALLOW → rattachement silencieux
        response = self.client.post('/api/v1/portal/submit/', {
        ...
        # ALLOW → rattachement silencieux sur le client existant : 201, 1 seul client
        self.assertEqual(response.status_code, 201)
        self.assertEqual(OwnerClient.objects.filter(email='duplicate@test.com').count(), 1)

    def test_portal_submit_conflict_only_on_phone(self):
        """Téléphone existant + noms différents + REQUIRE_OTP → identity_conflict au widget."""
        self.schema.opt = True
        self.schema.save()
        
        # self.lead (+2290197000000) existe déjà, sans nom stocké
        ...
        # Niveau 3 : noms absents sur le client existant → identity_conflict renvoyé au widget
        self.assertEqual(response.status_code, 201)
        self.assertTrue(response.data['identity_conflict'])
...
class AnalyticsSummaryTest(TestCase):
    ...
    def test_analytics_summary_empty(self):
        ...
        self.assertEqual(data['period_leads'], 0)
        self.assertEqual(data['period_verified_leads'], 0)
        self.assertEqual(data['top_clients'], [])
        self.assertIn('leads_series', data)
        total_leads = sum(h['count'] for h in data['leads_series'])
    ...
    def test_analytics_summary_with_leads(self):
        ...
        self.assertEqual(data['period_verified_leads'], 5)
        self.assertIsInstance(data['top_clients'], list)
        self.assertIsInstance(data['leads_series'], list)
    ...
    def test_leads_by_hour_last_24h(self):
        """leads_series en mode horaire (days=1) contient uniquement les dernières 24h."""
        ...
        # days=1 → série horaire sur les dernières 24h
        data = analytics_summary(self.user.id, days=1)
        leads_series = data['leads_series']
        
        total_in_24h = sum(entry['count'] for entry in leads_series)
        self.assertEqual(total_in_24h, 1)
    
    def test_leads_by_hour_data_structure(self):
        """Vérifier la structure de leads_series (mode horaire days=1)."""
        for i in range(3):
            OwnerClient.objects.create(
                owner=self.user,
                mac_address=f'AA:BB:CC:DD:EE:{i:02d}',
        ...
        # days=1 → 24 entrées horaires
        data = analytics_summary(self.user.id, days=1)
        leads_series = data['leads_series']
        
        self.assertEqual(len(leads_series), 24)
        if len(leads_series) > 0:
            entry = leads_series[0]
    ...
    def test_analytics_summary_endpoint(self):
        ...
        self.assertIn('period_leads', response.data)
        self.assertIn('period_verified_leads', response.data)
        self.assertIn('top_clients', response.data)
        self.assertIn('leads_series', response.data)
        
        self.assertEqual(response.data['total_leads'], 5)
        self.assertEqual(response.data['period_verified_leads'], 5)

    def test_analytics_summary_return_rate(self):
        """Le taux de retour est calculé sur les clients récemment vus."""
        now = timezone.now()
        for i in range(100):
            recognition_level = i % 10
            c = OwnerClient.objects.create(
                owner=self.user,
                mac_address=f'AA:BB:CC:DD:EE:{i:02d}',
                payload={'nom': f'User {i}'},
                email=f'user{i}@test.com',
            ...
            # Forcer last_seen pour que tous soient vus dans la période
            OwnerClient.objects.filter(pk=c.pk).update(last_seen=now)
        
        data = analytics_summary(self.user.id)
        
        self.assertEqual(data['total_leads'], 100)
        
        # recognition_level > 1 : niveaux 2-9 → 80 clients sur 100 vus → 80.0 %
        self.assertEqual(data['period_return_rate'], 80.0)
...
class ConflictManagementTest(TestCase):
    ...
    def test_conflict_alert_created_on_email_conflict(self):
        """Stratégie ALLOW + email dupliqué → rattachement silencieux, aucune ConflictAlert."""
        ...
        # ALLOW → rattachement silencieux : 201, aucune alerte créée
        self.assertEqual(response.status_code, 201)
        self.assertEqual(ConflictAlert.objects.count(), 0)

    def test_conflict_require_otp_strategy(self):
        """REQUIRE_OTP + email dupliqué + noms différents → identity_conflict renvoyé au widget."""
        self.schema.conflict_strategy = 'REQUIRE_OTP'
        self.schema.save()
        ...
        # Niveau 3 : noms absents sur le client existant → identity_conflict, pas d'OTP
        self.assertEqual(response.status_code, 201)
        self.assertTrue(response.data['identity_conflict'])
    ...
    def test_whatsapp_notification_triggered(self, mock_whatsapp):
        """WhatsApp déclenché quand identity_confirmed=True crée une ConflictAlert."""
        self.schema.conflict_strategy = 'REQUIRE_OTP'
        self.schema.save()
        ...
        # identity_confirmed=True → nouveau client + ConflictAlert + notification WhatsApp
        self.client.post('/api/v1/portal/submit/', {
            'public_key': self.public_key,
            'mac_address': '22:22:22:22:22:22',
