...
def detect_existing_client(
    ...
    # 3️⃣ Vérification par email (conflit potentiel)
    if email:
        client = OwnerClient.objects.filter(
            owner=form_schema.owner,
            email=email
    ...
    # 4️⃣ Vérification par phone (conflit potentiel)
    if phone:
        client = OwnerClient.objects.filter(
            owner=form_schema.owner,
            phone=phone
...
def _handle_contact_conflict(
    ...
    """
    Gère un conflit par email ou phone.
    Requiert vérification avant mise à jour.
    """
    ...
    if not verification_code:
        ...
        return {
            ...
            'message': f'Un compte existe déjà avec cet {conflict_field}. Un code de vérification a été envoyé.'
        }
    ...
    # C) Code validé → mise à jour autorisée
