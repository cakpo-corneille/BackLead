...
class FormSchema(DirtyFieldsMixin, models.Model):
    ...
    CONFLICT_STRATEGY_CHOICES = [
        ('ALLOW', 'Rattachement silencieux (log uniquement)'),
        ('REQUIRE_OTP', 'Demander une confirmation d\'identité (Oui/Non)'),
    ]
    ...
    conflict_strategy = models.CharField(
        ...
        default='ALLOW',
        help_text="Comportement lors d'un conflit de contact : silencieux ou confirmation d'identité."
    )
...
class OwnerClient(models.Model):
    ...
    first_name = models.CharField(
        ...
        blank=True,
        default='',
        help_text="Prénom extrait automatiquement du payload à l'ingestion."
    )
    last_name = models.CharField(
        max_length=150,
        blank=True,
        default='',
        help_text="Nom de famille extrait automatiquement du payload à l'ingestion."
    )
...
class ClientDevice(models.Model):
    """
    Historique des appareils (MAC + user_agent) rattachés à un client.
    Un tuple (client, mac_address) est unique.
    """
    ...
    def __str__(self):
        return f"{self.mac_address} — {self.client}"


class ConflictAlert(models.Model):
    """
    Historique des conflits détectés lors de la soumission du portail.
    Se déclenche uniquement sur des cas ambigus confirmés (réponse "Oui c'est moi"
    avec noms différents) ou en cas d'appareils multiples anormaux.
    """
    STATUS_CHOICES = [
        ('PENDING', 'En attente'),
        ('RESOLVED', 'Résolu'),
